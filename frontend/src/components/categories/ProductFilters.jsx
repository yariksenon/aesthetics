...
const ProductFilter = ({ onFilter }) => {
  ...
  // Стиль для ползунка
  const getTrackStyle = () => {
    const minPercent = ((filters.priceRange.min - priceLimits.min) / (priceLimits.max - priceLimits.min)) * 100;
    const maxPercent = ((filters.priceRange.max - priceLimits.min) / (priceLimits.max - priceLimits.min)) * 100;
  ...
  const resetFilter = (filterType) => {
    setFilters(prev => {
      const newValue = filterType === 'priceRange' 
        ? priceLimits
  ...
  const filterOptions = {
    ...
    shoeSizes: Array.from({length: 15}, (_, i) => 35 + i),
    sortOptions: [
  ...
  const renderSelectedFilters = () => {
    ...
    if (filters.clothingSizes.length > 0) {
      selected.push({
        type: 'clothingSizes',
        label: `Одежда: ${filters.clothingSizes.join(', ')}`,
        value: filters.clothingSizes
      });
    }
    ...
    if (filters.shoeSizes.length > 0) {
      selected.push({
        type: 'shoeSizes',
        label: `Обувь: ${filters.shoeSizes.join(', ')}`,
        value: filters.shoeSizes
      });
    }
    
    if (filters.priceRange.min !== priceLimits.min || filters.priceRange.max !== priceLimits.max) {
      selected.push({
        type: 'priceRange',
    ...
    if (filters.availability) {
      const avail = filterOptions.availabilityOptions.find(a => a.id === filters.availability);
      selected.push({
        type: 'availability',
        label: avail?.label,
  ...
  const FilterDropdown = ({ type, label, options, multiSelect = true }) => {
    ...
    const hasValue = multiSelect 
      ...
      : type === 'priceRange'
        ? filters.priceRange.min !== priceLimits.min || filters.priceRange.max !== priceLimits.max
        ...
        {isOpen && type === 'priceRange' && (
          <div className="absolute left-0 mt-2 bg-white shadow-lg rounded-lg p-4 w-72 border border-gray-200 z-20">
            {isLoadingPrices ? (
              ...
              <>
                <div className="mb-6">
                  <div className="relative h-1 bg-gray-200 rounded-full" style={getTrackStyle()}>
                    <input
                      type="range"
                      name="min"
                      min={priceLimits.min}
                    ...
                    <div 
                      ...
                      style={{ left: `${((filters.priceRange.min - priceLimits.min) / (priceLimits.max - priceLimits.min)) * 100}%` }}
                    />
                    <div 
                      className="absolute top-1/2 h-4 w-4 bg-black border-2 border-white rounded-full shadow-md transform -translate-y-1/2 -translate-x-1/2"
                      style={{ left: `${((filters.priceRange.max - priceLimits.min) / (priceLimits.max - priceLimits.min)) * 100}%` }}
                    />
                  </div>
                </div>
                ...
                {options.map(option => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="text-sm">{option.name || option.label || option}</span>
                ...
                {options.map(option => (
                  <div
                    ...
                    className={`flex items-center gap-3 cursor-pointer px-2 py-2 rounded-lg ${
                      filters[type] === option.id ? 'bg-gray-50' : 'hover:bg-gray-50'
                    }`}
                    onClick={() => toggleFilter(type, option.id)}
                  >
                    <div className={`h-4 w-4 rounded-full border ${
                      filters[type] === option.id ? 'border-black' : 'border-gray-300'
                    }`} />
                    <span className="text-sm">{option.label}</span>
                    {filters[type] === option.id && <FiCheck className="ml-auto" size={14} />}
                  </div>
                ))}
              </div>
            )}
  ...
  return (
    <div className="bg-white rounded-xl">
      <div className="flex flex-wrap gap-3 pb-4">
        ...
        <FilterDropdown type="shoeSizes" label="Размер обуви" options={filterOptions.shoeSizes} />
        <FilterDropdown type="priceRange" label="Цена" options={[]} multiSelect={false} />
        <FilterDropdown type="availability" label="Наличие" options={filterOptions.availabilityOptions} multiSelect={false} />
        <FilterDropdown type="sortBy" label="Сортировка" options={filterOptions.sortOptions} multiSelect={false} />
      </div>
