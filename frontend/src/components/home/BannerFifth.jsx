...
const slideData = [
  { text: "Форма для бега", about: "Для удобства и скорости", category: "running" },
  { text: "Тренировочная форма", about: "Комфорт и стиль", category: "training" },
  { text: "Форма для регби", about: "Защита и комфорт", category: "rugby" },
  { text: "Футбольное снаряжение", about: "Идеально для игры", category: "football" },
];
  ...
  const NavigationButtons = () => (
    <div className='flex mt-[1%] items-center space-x-2'>
      <button onClick={handlePrevSlide} className='cursor-pointer'>
        <img src={leftArrow} alt="Previous Slide" className='w-4 h-full sm:w-6 md:w-8 lg:w-10' />
      </button>
      <div className='flex items-center space-x-1'>
        <p className='text-[10px] sm:text-xs md:text-sm lg:text-base'>{currentSlide + 1}</p>
        <img src={slash} alt="Separator" className='w-3 h-full sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-6 lg:h-6' />
      ...
      <button onClick={handleNextSlide} className='cursor-pointer'>
        <img src={rightArrow} alt="Next Slide" className='w-4 h-full sm:w-6 md:w-8 lg:w-10' />
      </button>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-3 gap-4 mt-[5%]">
        ...
        {[BannerFifthGym, BannerFifthDayving].map((banner, index) => (
          <div key={index} className="group">
            <img
              ...
              onClick={() => handleClick(bannerData[index].category)}
            />
            <div>
            ...
            <div className='flex items-center transform -translate-x-5 group-hover:translate-x-0 transition-transform duration-300 ease-in-out'>
              <img src={shortLine} alt="Short Line Decoration" className="hidden group-hover:block w-[20%]" />
      ...
      <div className="col-span-2 relative h-[25vh] md:h-[40vh] lg:h-[50vh] xl:h-full group p-0 m-0 w-full">
        <div className="relative w-full h-full overflow-hidden">
          {slides.map((slide, index) => (
            <img
              ...
              onClick={() => handleClick(slideData[index].category)}
            />
          ))}
          <ProgressBar />
        </div>
        ...
        <div className='flex items-center transform -translate-x-5 group-hover:translate-x-0 transition-transform duration-300 ease-in-out'>
          <img src={longLine} alt="Long Line Decoration" className="hidden group-hover:block w-[20%]" />
          <p className='ml-[2%] hidden text-[10px] sm:text-xs md:text-sm lg:text-base text-gray-600 group-hover:block'>
            Купить
          </p>
        </div>
