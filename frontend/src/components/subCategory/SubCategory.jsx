...
const SubCategories = () => {
	...
	const fetchSubCategories = async () => {
		try {
			const response = await axios.get(
				'http://localhost:8080/api/v1/admin/subcategories'
			)
	...
	const validateForm = subCategory => {
		const errors = {}
		if (!subCategory.name || subCategory.name.length < 2)
			errors.name = 'Название должно быть минимум 2 символа'
		if (!subCategory.parent_id)
			errors.parent_id = 'Выберите родительскую категорию'
		return errors
	}
			...
			if (result.isConfirmed) {
				try {
					...
					const response = await axios.put(
						`http://localhost:8080/api/v1/admin/subcategories/${editingSubCategory.id}`,
						dataToSend
					)
			...
			if (result.isConfirmed) {
				try {
					const response = await axios.delete(
						`http://localhost:8080/api/v1/admin/subcategories/${subCategoryId}`
					)
					if (response.status === 200) {
						fetchSubCategories()
		...
		<div className='p-6 bg-gray-100 min-h-screen text-gray-800'>
			...
			{showAddForm && (
				<form
					...
					className='mb-6 p-4 bg-white shadow rounded-lg border border-gray-300'
				>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
						<div>
							<input
								...
								} rounded w-full focus:outline-none focus:ring-2 focus:ring-gray-600`}
							/>
							{formErrors.name && (
						...
						<div>
							<select
								...
								onChange={handleInputChange}
								className={`p-2 border ${
									formErrors.parent_id ? 'border-red-500' : 'border-gray-400'
								} rounded w-full focus:outline-none focus:ring-2 focus:ring-gray-600`}
							>
