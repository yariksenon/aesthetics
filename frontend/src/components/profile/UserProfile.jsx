...
const UserProfile = () => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.data.avatar_url) {
          ...
          // Дефолтный аватар
          setAvatar('https://ui-avatars.com/api/?name=' + 
                    encodeURIComponent(response.data.first_name + ' ' + response.data.last_name) + 
                    '&background=random');
        }
      } catch (err) {
  ...
  const handleUnsubscribe = async () => {
    try {
      const token = localStorage.getItem('authToken') || getCookie('auth_token');
      await axios.post('http://localhost:8080/api/v1/profile/unsubscribe', {}, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="flex items-center">
              ...
              <div className="flex-1 flex items-center">
                <span className={`font-medium ${profile.subscription ? 'text-green-600' : 'text-gray-600'}`}>
              ...
              <div className="space-y-2">
                {userAddress ? (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <p><span className="text-gray-600">Street:</span> {userAddress.street}</p>
                    <p><span className="text-gray-600">City:</span> {userAddress.city}</p>
                    <p><span className="text-gray-600">Postal Code:</span> {userAddress.postal_code}</p>
                    <p><span className="text-gray-600">Country:</span> {userAddress.country}</p>
                  </div>
                ) : (
